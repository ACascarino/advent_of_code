class Graph:
    ...
    def h(self, vertex1, vertex2):
        return 0  # vertex2.value - vertex1.value
    ...
    def find_shortest_path(self, start):
        ...
        f_score[start] = self.h(start, self.end_vertex)
...
starting_points = [x for x in mountain.vertices.values() if x.value == ord("a")]
paths = (mountain.find_shortest_path(x) for x in starting_points)
path_lens = [len(x) - 1 for x in paths if x is not None]
print(min(path_lens))
